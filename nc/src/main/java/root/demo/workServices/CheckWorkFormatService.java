...
@Service
public class CheckWorkFormatService implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		List<FormSubmissionDto> forma = (List<FormSubmissionDto>) execution.getVariable("format");
		...
		for (FormSubmissionDto formField : forma) {
			...
			if(formField.getFieldId().equals("reviewFormFormat")) {
...
//		korisnik = krepo.getOne(idk);
		execution.setVariable("reviewFormFormat", potvrda);	
		System.out.println("BRAVO DOKTORE NESTO JE URADJENO SADA");
	}
