...
@Service
public class SavingMajorChangesService implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		List<FormSubmissionDto> casopisForma = (List<FormSubmissionDto>) execution.getVariable("workMajor");
		Long nid = (Long) execution.getVariable("nrId"); 
		NaucniRad nr = nrrepo.getOne(nid);
		
		for (FormSubmissionDto formField : casopisForma) {			
			if (formField.getFieldId().equals("majorFormPdf")) {
