...
//			Task task = taskService.createTaskQuery().processInstanceId(processInstance.getId()).list().get(0);
	...
	public @ResponseBody ResponseEntity<List<NaucnaOblast>> getNaucneOblasti() {
		List<NaucnaOblast> oblasti = norepo.findAll();
		return new ResponseEntity<List<NaucnaOblast>>(oblasti, HttpStatus.OK);
	}
	
	@PostMapping(path = "/confirmAccount/{processId}", produces = "application/json")
    public @ResponseBody ResponseEntity<?> activateAcc(@PathVariable String processId) {
        Task task = taskService.createTaskQuery().processInstanceId(processId).list().get(0);
//        Task task = taskService.createTaskQuery().taskId("Task_1s7vfn2").singleResult();
        taskService.complete(task.getId());
        return new ResponseEntity<>(HttpStatus.OK);
    }
	
	@GetMapping(path = "/getForConfirmation", produces = "application/json")
    public @ResponseBody ResponseEntity<FormFieldsDto> getForConfirmation() {

        List<Task> tasks = taskService.createTaskQuery().taskCandidateGroup("ncadmin").list();
        FormFieldsDto formFieldsDto = new FormFieldsDto();
        for(Task task: tasks){
            if(task.getName().equals("ConfirmReviewer")) {
	...
	@PostMapping(path = "/postRevConfirmation/{taskId}", produces = "application/json")
    public @ResponseBody ResponseEntity<?> postRevConfirmation(@RequestBody List<FormSubmissionDto> dto, @PathVariable String taskId) {
		HashMap<String, Object> map = this.mapListToDto(dto);
		
		
		Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
		String processInstanceId = task.getProcessInstanceId();
		runtimeService.setVariable(processInstanceId, "revconfirmation", dto);
		formService.submitTaskForm(taskId, map);
        return new ResponseEntity<>(HttpStatus.OK);
    }
	...
	public @ResponseBody ResponseEntity<List<TaskDto>> getAdminTasks(){
		...
		tasks.addAll(taskService.createTaskQuery().processDefinitionKey("Process_Registracija").taskAssignee("demo").list());
