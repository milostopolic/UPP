...
@Component({
  selector: 'app-changes-major',
  templateUrl: './changes-major.component.html',
  styleUrls: ['./changes-major.component.css']
})
export class ChangesMajorComponent implements OnInit {
  ...
  onSubmit(value, form) {
    ...
    this.workService.uploadFile(this.fileToUpload).subscribe(response => {
      ...
      o.push({ fieldId: "majorFormPdf", fieldValue: response });

      // if (this.selectedItems.length != 1) {
      //   alert("Molimo vas da odaberete jedan casopis");
      // } else {
        for (var property in value) {
          if (property != "majorFormPdf") {
        ...
        console.log(o);
          ...
          let x = this.workService.postMajor(o, this.formFieldsDto.taskId);
          x.subscribe(
            res => {
              console.log(res);
